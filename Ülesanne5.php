...
<body>
    ...
    05.05.2026
    */
    $nimed = array("Emily","Gerson","Kati","Mari","Penny","Pam","Abigail","Robin");
    ...
    // Source - https://stackoverflow.com/a/838239
    // Posted by Unknown, modified by community. See post 'Timeline' for change history
    ...
    usort($nimed,'sortByLength');
    foreach($nimed as $nimi) {
        echo "$nimi <br>";
    }
    echo "<hr>";
    $counted_names = count($nimed);
    for($i=0;$i <3; $i +=1) {
        echo $nimed[$i]."<br>";
    }
    echo"<hr>";
    echo $nimed[$counted_names-1];
    echo "<hr>";
    echo $nimed[rand(0,$counted_names-1)]."<br> <hr>";
    ...
    $vin_codes = array("1GKS1GKC8FR966658", "1FTEW1C87AK375821", "1G4GF5E30DF760067", "1FTEW1CW9AF114701", "WAUGGAFC8CN433989", "3G5DA03E83S704506", "4JGDA2EB0DA207570", 
    ...
    $counted_cars = count($cars);
    $counted_vin_codes = count($vin_codes);
    if( $counted_cars == $counted_vin_codes ) {
        echo"Massiviid on võrdsed <br>";
    }
    else {
        echo"Massiviid pole võrdsed <br>";
    }
    $toyota = 0;
    $audi = 0;
    foreach($cars as $car){
        if(str_contains("$car","Toyota")) {
            $toyota +=1;
        }
        if(str_contains("$car","Audi")) {
            $audi +=1;
        }
    }
    echo"Toyotade arv: $toyota, Audide arv: $audi <br>";
    foreach($vin_codes as $vin_code){
        $vin_len = strlen($vin_code);
        if($vin_len < 17) {
            echo "$vin_code <br>";
        }
    }
    $palgad_2018 = array(1220,1213,1295,1312,1298,1354,1296,1286,1292,1327,1369,1455);
    $average = array_sum($palgad_2018)/count($palgad_2018);
    echo"Keskmine palk on $average <br> <hr>";
    ?>

    <form action="">
        Eemalda firma <input type="text" name="firma_eemaldada"><br>
